...
class Waindigo_Install
{
    ...
    {
        if (self::_isAddOnInstalled('Waindigo_InstallUpgrade')) {
            if (method_exists('Waindigo_InstallUpgrade_ControllerHelper_InstallUpgrade', 'setFileName')) {
                Waindigo_InstallUpgrade_ControllerHelper_InstallUpgrade::setFileName($resourceUrl);
            }
        }
    }
        ...
        if (isset($this->_data['addon_id'])) {
            if (XenForo_Application::$versionId < 1020000 &&
                ...
                XenForo_Application::setSimpleCacheData(
                    Waindigo_Listener_InitDependencies::JUST_INSTALLED_SIMPLE_CACHE_KEY, $justInstalled);
    ...
    protected function _updateJustUninstalled()
    {
        if (isset($this->_data['addon_id'])) {
            if (XenForo_Application::$versionId < 1020000 &&
                ...
                XenForo_Application::setSimpleCacheData(
                    Waindigo_Listener_InitDependencies::JUST_UNINSTALLED_SIMPLE_CACHE_KEY, $justUninstalled);
            } elseif (XenForo_Application::$versionId >= 1020000) {
                XenForo_Application::defer('Waindigo_Deferred', 
        ...
        if (XenForo_Application::$versionId > 1020000) {
            ...
            if ($hint) {
                if (isset($installerWaindigoListeners[$hint])) {
                    $installerWaindigoListeners = array_merge($installerWaindigoListeners, 
                        $installerWaindigoListeners[$hint]);
                }
            }
        }
