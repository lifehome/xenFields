...
/**
 * Data writer for custom social forum fields.
 */
class Waindigo_CustomFields_DataWriter_SocialForumField extends Waindigo_CustomFields_DataWriter_AbstractField
{
     ...
     * field.
     *
     * @return Waindigo_CustomFields_Definition_SocialForumField
     */
    public function getFieldDefinition()
    {
        return new Waindigo_CustomFields_Definition_SocialForumField();
    }
    ...
    protected function _getFields()
    {
        ...
        $fields['xf_social_forum_field']['viewable_information'] = array(
            'type' => self::TYPE_UINT,
            'default' => 0
        );
        return $fields;
    ...
    protected function _getExistingData($data)
    {
        ...
        return array(
            'xf_social_forum_field' => $this->_getFieldModel()->getSocialForumFieldById($id)
        );
    }
    ...
    protected function _verifyFieldId(&$id)
    {
        ...
        if ($id !== $this->getExisting('field_id') && $this->_getFieldModel()->getSocialForumFieldById($id)) {
    ...
    protected function _preSave()
    {
        ...
        if ($this->isUpdate() && $this->isChanged('field_type')) {
            $typeMap = $this->_getFieldModel()->getSocialForumFieldTypeMap();
    ...
    protected function _postSave()
    {
        if (!$this->getOption(self::OPTION_MASS_UPDATE)) {
            ...
            if ($this->isChanged('display_order') || $this->isChanged('field_group_id')) {
                $this->_getFieldModel()->rebuildSocialForumFieldMaterializedOrder();
            }

            $this->_rebuildSocialForumFieldCache();
        }
    }
    ...
    protected function _postDelete()
    {
        ...
        $this->_db->delete('xf_social_forum_field_value', 'field_id = ' . $this->_db->quote($fieldId));
        // note the social forum caches aren't rebuilt here; this shouldn't be
    // an issue as we don't enumerate them
    }
    ...
    {
        return $this->_getFieldModel()->getSocialForumFieldTitlePhraseName($id);
    }
    ...
    {
        return $this->_getFieldModel()->getSocialForumFieldDescriptionPhraseName($id);
    }
    ...
    {
        return $this->_getFieldModel()->getSocialForumFieldChoicePhraseName($fieldId, $choice);
    }

    protected function _rebuildSocialForumFieldCache()
    {
        return $this->_getFieldModel()->rebuildSocialForumFieldCache();
    }
